...
class ProductDetailsSpider(scrapy.Spider):
    ...
    def __init__(self, reviews_option=1, count_max=None, *args, **kwargs):
    ...
    def parse_product(self, response):
        ...
        if self.COUNTER >= self.COUNT_MAX:
            raise scrapy.exceptions.CloseSpider(
                    reason='COUNT_MAX value reached - {} items'.format(self.COUNT_MAX))
        ...
        try:
            product_id = response.url.split('/')[5]
        ...
        l.add_xpath('title', '//*[@id="listing-page-cart"]/div[@class="wt-mb-xs-2"]/h1/text()')

        # Get the product price
        l.add_xpath('price', '//*[contains(@data-buy-box-region, "price")]//p')

        # Get the product sales
        l.add_xpath('sales', '//div[@id="listing-page-cart"]/div[1]/div/div[2]/div/span[@class="wt-text-caption "]/text()')

        # Get the product URL (ex: www.etsy.com/hk-en/listing/930730854/246-tcw-cushion-rensu-cut-moissanite)
        l.add_value('url', '/'.join(response.url.split('?')[0].split('/')[2:7]))
